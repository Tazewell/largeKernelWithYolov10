...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, ReparamLargeKernelConv, autopad
...
class RepLKBlock(nn.Module):
    def __init__(self, in_channels, dw_channels, block_lk_size, small_kernel, drop_path=0.3):
        super().__init__()
        self.pw1 = CBA(in_channels, dw_channels, 1, 1, 0, g=1)
        self.pw2 = CBA(dw_channels, in_channels, 1, 1, 0, g=1)
        # 大卷积核
    ...
    def forward(self, x):
        ...
        out = self.pw1(out)  # 向上调整通道
        out = self.large_kernel(out)  # 大卷积核
        out = self.lk_nonlinear(out)  # 激活函数
        out = self.pw2(out)  # 向下调整通道
...
class CBA(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, a=nn.ReLU()):
        """初始化类"""
